# Narkosegerät - Perseus

---
## 🫁 GASE

#### O₂ (Sauerstoff)  
- **Normwerte:**  
	→ FiO₂: Raumluft 0,21; klinisch meist 0,3–0,5 unter Narkose  
	→ EtO₂: ca. 0,16–0,18 bei FiO₂ 0,21  
- Differenz FiO₂–EtO₂:  
	→ normal ≈ 0,03–0,05 = normale alveoläre Aufnahme  
	→ wenn Unterschied kleiner (<0,02) → Shunt/Diffusionsstörung  
	→ wenn Unterschied größer (>0,06) → hohe O₂-Aufnahme, z. B. bei Hypermetabolismus  

> [!Beispiel]  
> **Shunt-Schätzung:**  
> FiO₂ 0,5 – EtO₂ 0,45 = 0,05 → normale Oxygenierung.  
> FiO₂ 0,5 – EtO₂ 0,48 = 0,02 → Hinweis auf relevanten Shunt.

#### CO₂ (Kohlendioxid)  
- **Normwerte:**  
	→ EtCO₂: 35–45 mmHg (≈ 4,6–6,0 kPa)  
	→ FiCO₂: quasi 0 mmHg  
- **FiCO₂ erhöht:**  
	→ Rebreathing, also Kalk wechseln
	→ wenn >5 mmHg
- **abrupter Abfall EtCO₂ >10 mmHg**  
	→ Kreislaufstillstand oder massiver HZV-Abfall

#### Narkosegase & MAC
- MAC-Werte (bei 40 Jahren, 1 atm):
	  - Sevofluran: 2,0 Vol%
	  - Desfluran: 6,0 Vol%
	  - Isofluran: 1,15 Vol%
- MAC sinkt mit Alter (ca. –6 % pro Dekade)
- Inspiratorisch vs. exspiratorisch: Uptake sichtbar (Differenz 0,2–0,5 Vol% üblich)

---
## 📈 KURVEN & VOLUMINA  

#### CO₂-Kurve (=Kapnographie)
- Normwert Plateau: 35–45 mmHg
- „Haifischflosse“  
	→ Bronchospasmus  
- Plötzlicher Abfall  
	→ Kreislaufstillstand  

#### Druckkurve
- Ppeak: <30 cmH₂O (bei Schutzbeatmung)
- Pplat: <25 cmH₂O
- PEEP: 5–8 cmH₂O üblich
- Differenz Ppeak–Pplat = Resistance (indirekt)

#### Flowkurve
- Exspiration vollständig → Flow kehrt zur Null-Linie zurück
- Restflow → Auto-PEEP
- Spontanatmung → unregelmäßige Flowmuster

#### VT & MV
- VT: 6–8 ml/kg KG (ideales KG)
- MV: 5–8 l/min bei Erwachsenen
- Schwankungen → Spontanatmung oder Leckagen  

---
## 🧮 ERWEITERE PARAMETER

#### Cdyn (dynamische Compliance)
- Normwert: 50–100 ml/cmH₂O bei gesunden Erwachsenen
- Abfall <30 ml/cmH₂O 
	→ Hinweis auf restriktive Pathologie
- Verlaufskontrolle entscheidend bei Lagerung/Rekrutierung
$$
C_{dyn} = \frac{V_T}{P_{peak} - PEEP}
$$
#### MV × CO₂ (=Ventilationseffizienz)
- **Normwert**: CO₂-Elimination ≈ 200 ml/min bei Erwachsenen
- Hohe MV, aber niedrige CO₂-Elimination → Totraumventilation ↑
- Besonders relevant bei ARDS/[COPD](COPD.md)

#### O₂-Uptake (=Sauerstoffaufnahme=VO₂)
$$
VO_2 = (FiO_2 - EtO_2) \times MV
$$
- **Normwert VO₂**: 
	→ 3–4 ml/kg/min  
	→ ≈ 250 ml/min bei 70 kg  
- Sinkt bei:  
	→ Hypothermie  
	→ tiefer Narkose  
	→ Kreislaufdepression. 
- Steigt bei:  
	→ Fieber  
	→ Sepsis  
	→ Hypermetabolismus  

#### ΔVT (Delta Tidalvolumen)
- **Normwert**:  
	→ Variabilität 10–20 % bei Spontanatmung
- ΔVT nahe null  
	→ tiefe Sedierung oder Relaxation  
- Hohe Variabilität  
	→ aktiver Atemantrieb, gute Synchronität  

---

<details>
<summary>🩺 Praxisbeispiele</summary>

- **COPD-Patient:**  
  - EtCO₂ ↑ mit breitem Plateau → CO₂-Retention  
  - MV × CO₂ ineffizient → Totraum ↑  
  - ΔVT hoch → unregelmäßige Spontanatmung unter Narkose  

- **Narkoseausleitung:**  
  - EtO₂ steigt, Uptake sinkt → Wash-out der Narkosegase  
  - ΔVT nimmt zu → Rückkehr der Spontanatmung  
  - EtCO₂-Trend stabilisiert sich → Kreislauf stabil  

- **Lagerungseffekte (z. B. Bauchlagerung):**  
  - Cdyn ↑ nach Rekrutierung → bessere Compliance  
  - FiO₂–EtO₂-Differenz ↓ → verbesserte Oxygenierung  
  - O₂ Uptake ↑ → gesteigerte alveoläre Aufnahme  

</details>

---
<details>
<summary>🔤 Abkürzungen</summary>

<table>
  <thead>
    <tr>
      <th>Abkürzung</th>
      <th>Bedeutung</th>
    </tr>
  </thead>
  <tbody>
    <tr><td>FiO₂</td><td>Inspiratorische Sauerstofffraktion</td></tr>
    <tr><td>EtO₂</td><td>Exspiratorische Sauerstofffraktion</td></tr>
    <tr><td>EtCO₂</td><td>Endexspiratorisches Kohlendioxid</td></tr>
    <tr><td>FiCO₂</td><td>Inspiratorisches Kohlendioxid</td></tr>
    <tr><td>MAC</td><td>Minimal Alveolar Concentration</td></tr>
    <tr><td>VT</td><td>Tidalvolumen</td></tr>
    <tr><td>MV</td><td>Atemminutenvolumen</td></tr>
    <tr><td>Ppeak</td><td>Spitzendruck</td></tr>
    <tr><td>Pplat</td><td>Plateaudruck</td></tr>
    <tr><td>PEEP</td><td>Positive Endexpiratory Pressure</td></tr>
    <tr><td>Cdyn</td><td>Dynamische Compliance</td></tr>
    <tr><td>MV × CO₂</td><td>Ventilationseffizienz (CO₂-Elimination)</td></tr>
    <tr><td>O₂ Uptake</td><td>Sauerstoffaufnahme / VO₂</td></tr>
    <tr><td>ΔVT</td><td>Delta Tidalvolumen (Variabilität)</td></tr>
  </tbody>
</table>

</details>

<details>
<summary>📚 Quellen</summary>

- West JB: *Respiratory Physiology – The Essentials*, 10th Edition  
- Miller RD: *Miller’s Anesthesia*, 9th Edition  
- Eger EI et al.: MAC and anesthetic potency, *Anesth Analg*  
- Nunn JF: *Applied Respiratory Physiology*  
- Dräger Perseus A500 – Technical Manual & Clinical Guide  
- Gattinoni L, Marini JJ: ARDS mechanics and gas exchange, *Intensive Care Med*  

</details>

<details>
<summary>🏷️ Tags</summary>

- Anästhesie  
- Narkosegerät  
- Beatmung  
- Dräger Perseus  
- Monitoring  
- Physiologie  
- Intensivmedizin  
- Operation  
- Atemmechanik  

</details>
